...
export function HeroVisual({ className }: { className?: string }) {
  return (
    <div className={cn("relative mx-auto w-full max-w-lg lg:max-w-none", className)}>
      ...
      <div className="animate-float-tilt absolute -right-2 top-0 z-20 flex items-center gap-2.5 rounded-2xl border border-accent/30 bg-surface px-4 py-3 depth-elevated sm:right-0 [animation-delay:0.6s]">
        <span className="grid h-8 w-8 place-items-center rounded-full bg-accent text-accent-foreground shadow-[0_4px_12px_rgba(198,255,0,0.35)]">
      ...
      <Card3D className="relative z-10" intensity={12} float>
        <div className="depth-elevated-dark overflow-hidden rounded-[var(--radius-bento)] border border-white/10 bg-[#0a0a0a]">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            ...
            <span className="text-[11px] font-medium text-white/40">reload/app · main</span>
            <div className="flex gap-1.5">
              <span className="rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-bold text-accent">TS</span>
            </div>
          </div>

          <div className="space-y-1 px-4 py-5 font-mono text-[11px] leading-relaxed sm:px-5 sm:py-6 sm:text-xs">
      ...
      <div className="animate-float-tilt absolute -bottom-4 -left-2 z-20 rounded-2xl border border-border bg-surface px-4 py-3 depth-elevated sm:left-0 [animation-delay:1.2s]">
      ...
      <div className="animate-float absolute -bottom-2 right-4 z-20 flex items-center gap-2 rounded-full border border-accent/30 bg-accent-subtle px-3 py-1.5 shadow-[0_8px_24px_-8px_rgba(198,255,0,0.35)] [animation-delay:0.3s]">
